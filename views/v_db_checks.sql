...
# Author: YJ
# Date: 2016.07.13
# Desc: check major status in db
#
CREATE OR REPLACE
ALGORITHM=UNDEFINED
DEFINER = 'root'@'localhost'
SQL SECURITY INVOKER
VIEW sys.`v$db_checks`
AS
SELECT 'Connection' AS category
      ,'Refued Connection' AS division
      ,'연결 실패 비율' AS description
...
SELECT 'Connection' AS category
      ...
      ,'동시 접속이 가능한 최대 수치 대비 연결된 Thread의 비율' AS description
...
SELECT 'Connection' AS category
      ...
      ,'동시 접속이 가능한 최대 수치 대비 가장 많이 접속이 사용됐던 비율' AS description
      ,round(s.variable_value / v.variable_value * 100, 1) AS current_percent
      ,CASE 
       WHEN (s.variable_value / v.variable_value * 100
            ) > 90
...
SELECT 'Index' AS category
      ...
      ,'Table Full scan을 통해 Aaccess한 Row의 비율' AS description
      ,round(
             (
...
SELECT 'Temporary Table' AS category
      ...
      ,'임시테이블이 물리적 Disk에 생성된 비율' AS description
...
SELECT 'InnoDB Cache' AS category
      ...
      ,'InnoDB Buffer pool 작성전에 대기하는 비율' AS description
...
SELECT 'InnoDB Cache' AS category
      ...
      ,'InnoDB Buffer pool 에서 읽어 오는 비율' AS description
      ,round(100 -
...
SELECT 'Query Cache' AS category
      ...
      ,'Query Cache 사용율 (25%미만: query_cache_size 축소 권장, 80%초과: Qcache_lowmem_prunes가 50을 초과하면 query_cache_size 증가 권장)' AS description
      ,round(100 -
...
SELECT 'Open Table' AS category
      ...
      ,'오픈된 적이 있는 테이블 중에 현재 오픈 돼 있는 테이블 비율' AS description
